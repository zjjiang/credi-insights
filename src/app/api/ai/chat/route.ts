...
const tools: OpenAI.Chat.ChatCompletionTool[] = [
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "查询所有可用的分类列表，用于创建规则或分配分类时查找 categoryId",
  ...
  },
  {
    ...
    function: {
      name: "create_rule",
      description: "创建一条分类规则，上传账单时 AI 会按规则自动分类",
      parameters: {
        type: "object",
        properties: {
          ...
          description: {
            type: "string",
            description: '自然语言描述规则，如"商户包含美团 → 餐饮"',
          },
...
export async function POST(request: Request) {
  try {
    ...
    const systemPrompt = `你是一个信用卡账单分析助手。你可以查询交易数据、回答消费问题、批量修改交易分类、查询分类列表、创建分类规则。创建规则前先用 list_categories 获取分类ID。${
      ...
      async start(controller) {
        ...
        for (let i = 0; i < 5; i++) {
          ...
          for (const tc of toolCalls) {
            ...
            const input = JSON.parse(tc.function.arguments) as Record<
              string,
              unknown
            >;
