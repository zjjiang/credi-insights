...
describe("buildDateRangeSearch", () => {
  ...
  it("builds inclusive range with subject and exclusive before (end+1d)", () => {
    const q = buildDateRangeSearch("2026-08-07", "2026-08-10", subject);
  ...
  it("handles a single-day range (start === end)", () => {
    const q = buildDateRangeSearch("2026-08-10", "2026-08-10", subject);
  ...
  it("throws on invalid date format", () => {
    expect(() =>
      buildDateRangeSearch("2026/08/10", "2026-08-10", subject),
    ).toThrow();
  });

  it("throws when start is after end", () => {
    expect(() =>
      buildDateRangeSearch("2026-08-11", "2026-08-10", subject),
    ).toThrow();
  });
});
