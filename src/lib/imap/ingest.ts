...
export async function runDailyIngest(): Promise<IngestResult> {
  ...
  const result: IngestResult = { fetched: emails.length, inserted: 0, skipped: 0 }
  if (emails.length === 0) return result

  const newlyInsertedIds: string[] = []

  for (const email of emails) {
    const { transactions } = parseDailyEmail({ text: email.text, html: email.html })

    for (const tx of transactions) {
      const outcome = await upsertDailyTransaction(tx)
      if (outcome.inserted) {
        result.inserted += 1
        newlyInsertedIds.push(outcome.id)
      } else {
        result.skipped += 1
      }
    }
...
async function classifyNewTransactions(ids: string[]): Promise<void> {
  try {
    ...
    const classifications = await classifyTransactions(
      txs.map((t) => ({ id: t.id, merchant: t.merchant, amount: Number(t.amount) })),
      rules.map((r) => ({
